...
<div class="row">
    <div class="col-md-12">
        <form action="{{route('buy_factor.update',$factor->buy_factor_id)}}" method="post" id="frm">
            {{csrf_field()}}
            <div class="form-row">

                <div class="form-group col-md-2 col-lg-2 col-sm-12">
                    <label for="stack_name">شماره فکتور:*</label>
                    <input type="text" placeholder="شماره فکتور" required name="sale_factor_code"
                           id="sale_factor_code" value="{{ $factor->factor_code }}" class="form-control select2_1 sale_factor_code">
                ...
                <div class="form-group col-md-2">
                    ...
                    <select name="company_name" id="company_name" class="form-control select2_1 company_name">
                        ...
                        @foreach($companyes as $companye)
                            <option value="{{ $companye->company_id }}" {{ $factor->company_id ==$companye-> company_id ? "selected" : "" }} >{{ $companye->company_name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-2">
                    ...
                    <select name="stack_name" id="stack_name" class="form-control select2_1 stack_name">
                        @foreach($stores as $store)
                            <option value="{{ $store->store_id }}" {{ $factor->store_id == $store->store_id ? "selected" : "" }}>{{ $store->store_name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-2 col-lg-2 col-sm-12" >
                    ...
                    <input type="date" class="form-control required" value="{{ $factor->buy_date }}" placeholder="روز/ماه/سال"
                ...
                <div class="form-group col-md-2 col-lg-2 col-sm-12">
                    ...
                    <div class="input-group d-flex">
                        <input type="text" value="{{ $factor->currency_rate }}" required name="currency_rate"
                               id="currency_rate"
                        ...
                        <select class="form-control no-padding" name="currency_id"  id="currency_id">
                            @foreach($currencies as $currency)
                                <option
                                        @if($factor->currency_id == $currency->currency_id) selected @endif
                            ...
                            <tr id="row_{{ $key+1 }}">
                                ...
                                <td>
                                    <input type="text" readonly data-field-name="product_name" name="product_name[]"
                                           id="product_name_{{ $key+1 }}" class="form-control input-sm autocomplete_txt  "
                                           autocomplete="off" value="{{ $p->product_name }}">
                                    <input type="hidden" data-field-name="product_id" name="product_id[]" id="product_id_{{ $key+1 }}"
                                           class=" form-control input-sm autocomplete_txt"  value="{{ $p->product_id }}" autocomplete="off">
                                    <input type="hidden" data-field-name="product_store_id" name="product_store_id[]"
                                           id="product_store_id_{{ $key+1 }}"  value="{{ $p->buy_product_id }}" class="  form-control input-sm autocomplete_txt"
                                           autocomplete="off">
                                </td>
                                ...
                                <td>
                                    <select  data-field-name="product_unit" name="product_unit_exchange[]"
                                        ...
                                        <option value="{{ $p->exchange_unit_id }}">{{ get_unit($p->exchange_unit_id)->first()->unit_name }}</option>
                                    </select>
                                ...
                                <td><input type="text" data-field-name="pquantity" name="product_quantity[]" required
                                           id="product_quantity_{{ $key+1 }}" value="{{ $p->quantity }}"
                                           class="form-control input-sm  autocomplete_txt quantity amount  qty"></td>

								   <td><input type="text" data-field-name="product_buy_price" name="product_bprice[]"  value="{{ $p->buy_price }}"
                                           id="product_buy_{{ $key+1 }}" class="form-control input-sm autocomplete_txt buy_price qty"></td>

                                <td><input type="text" data-field-name="ptotal_amount" name="product_total[]" value="{{ $p->quantity * $p->buy_price  }}"
                                           id="product_total_{{ $key+1 }}" class="form-control input-sm autocomplete_txt amount_total">
                                </td>
                        ...
                        <tr>
                            ...
                            <td style="border: none;">
                                <div class="form-group required" id="form-discount-error">
                                    ...
                                    <input type="text" name="discount" value="{{ $factor->discount }}" id="discount" class="form-control required"
                            ...
                            <td style="border: none;">
                                <div class="form-group required" id="form-pr_payment-error">
                                    ...
                                    <input type="text" name="pr_payment" id="pr_payment" class="form-control required"
                                           title="قیمت پرداختی" value="{{ $factor->current_payment }}">
                            ...
                            <td style="border: none;">
                                <div class="form-group required" id="form-pr_total-error">
                                    ...
                                    <input type="text" name="pr_total" id="pr_total" class="form-control total required"
                                           title="قیمت کل"value="{{ $factor->total_payment }}">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        function handleTotalRow() {
            ...
            var amount = parseInt(tr.find('.quantity').val());

            var price = parseInt(tr.find('.buy_price').val());

            var total_amount = parseInt(amount * price);
    ...
    $(document).ready(function () {
        ...
        $('.stack_name').change(function(){
            ...
            if (confirm("با تغیر گدام اجناس کامل حذف میشود ایا حاضرید؟")){
