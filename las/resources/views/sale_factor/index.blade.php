<div class="col-md-12">
        ...
        <div class="col-md-6 d-flex" style="justify-content: flex-end">
            ...
            <div class="input-group">
                ...
                <select class="form-control" id="search-reason">
                    ...
                    <option value="byCustomerCode">جستجو با کد نمبر مشتری</option>
                </select>
            </div>
    ...
    <table id="example" class="table table-striped table-bordered display" style="width:100%">
        ...
        @foreach($sale_factors as $sale_factor)
            ...
            <tr>
                ...
                <td>{{$sale_factor->currency_name}}</td>
                <td>{{ number_format($sale_factor->recieption_price) }}</td>
                <td>{{ number_format($sale_factor->total_price - $sale_factor->recieption_price - $sale_factor->discount) }}</td>
                <td>{{ number_format($sale_factor->discount) }}</td>
                <td>{{ number_format($sale_factor->total_price) }}</td>
                <td>{{$sale_factor->sale_date}}</td>
                <td id="customer_id">

                    {{ isset($sale_factor->customer_name) ? $sale_factor->customer_name : $sale_factor->name  }}
                ...
                <td style="text-align: center" id="operations">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li> <a  target="_blank" href="{{ route('sale_factor.print',$sale_factor->sale_factor_id) }}" class="btn btn-link btn-block btn-sm "  ><i class="fa fa-print"></i> پرنت </a>
                            </li>
                            <li>  <a href="javascript:ajaxLoad('sale_factor/update/{{ $sale_factor->sale_factor_id }}')" class="btn btn-link btn-block btn-sm " ><i class="glyphicon glyphicon-edit"></i>ویرایش </a>
                            </li>
                            <li> <a href="javascript:ajaxLoad('sale_factor/returnFactor/{{ $sale_factor->sale_factor_id }}')" class="btn btn-link btn-block btn-sm " ><i class="fa fa-reply"></i>   بازگشت اجناس </a>
                            </li>
                            <li><a href="javascript:ajaxLoad('{{ route('sale_factors.customerPaymentList',$sale_factor->sale_factor_id) }}')" class="btn btn-link btn-block btn-sm " ><i class="fa fa-money"></i>  لیست پرداختی </a>
                            </li>
                            <li> <button sale_factor_id="{{ $sale_factor->sale_factor_id }}" class="btn btn-link btn-block btn-sm showModel" ><i class="glyphicon glyphicon-eye-open"></i>جزییات </button>
                            </li>
                            <li> <a class="btn btn-link btn-block btn-sm "  href="javascript:if(confirm('Are you sure you want to delete this?')) ajaxDelete('{{route('sale_factor.delete',$sale_factor->sale_factor_id)}}','{{csrf_token()}}')">
    ...
    <!-- The Modal -->
    <div id="myModal" class="drop-modal">
                        ...
                        success: function (response) {
                            ...
                            $.each(response.data, function (i, item) {
                                trHTML += '<tr>' +
                                    ...
                                    '<td>'+ (item.total_price - item.recieption_price - item.discount )+ '</td>' +
                                    '<td>'+ (item.discount )+ '</td>' +
